...
//                UserInfoManager.sendPong(ctx.channel());
            ...
            case ChatCode.JOIN_CODE: //普通的消息留给MessageHandler处理
                if(UserInfoManager.joinAroom(channel,json.getString("roomnum"))){
                    return;
                }else {
                    logger.warn("加入聊天室出现异常请检查");
                }
        ...
        //pipline的原因，我们可能有一个链式的处理队列，这个队列有头和尾之分，那么消息通常从头处理器进入。
