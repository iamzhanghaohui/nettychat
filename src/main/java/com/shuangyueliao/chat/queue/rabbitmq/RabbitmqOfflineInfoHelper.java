...
//        }
    ...
    @Override
    public Boolean addNewqueue(Account account) throws IOException {
        rabbitmqChannel.queueDeclare("queue_" + account.getId(), false, false, false, null);
        rabbitmqChannel.queueBind("queue_" + account.getId(), EXCHANGE_NAME, "p2p." + account.getId());
        return true;
    }
    @Override
    public Boolean setQueueBindRoom(UserInfo userInfo) throws IOException {
        rabbitmqChannel.queueBind("queue_" + userInfo.getId(), EXCHANGE_NAME, "group." + userInfo.getGroupNumber());
